...
class MY_Model extends CI_Model
{
	...
	*/
	...
	{
		$list = $this->db->list_fields($this->_table);
	...
	*/
	...
	public function init_upload_library()
	{	
		$this->load->library('upload');
		//following preference MAY BE loaded from general config?
		$upload['upload_path']          = './attachments/';
		$upload['allowed_types']        = 'gif|jpg|png';
		$upload['max_size']             = 1000; //1000kb 1mb
		$upload['max_width']            = 0; // 0 = no limit
		$upload['max_height']           = 0;			
		$this->upload->initialize($upload);			
	}
	
	function upload_library_error_msg_handler( $msg , $delimiter )
	{
		$upload_error_arr = explode( $delimiter , $msg );
		$row=1;
		foreach( $upload_error_arr AS $val ){
			if($val!='')
				$upload_error_str .= "<p>#".$row." ".$val."</p>" ;
				
			$row++;
		}

		return $upload_error_str ; 
	}	
}
